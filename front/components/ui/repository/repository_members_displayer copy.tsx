...
import { deleteUserFromRepository, getRepositoryMembers } from "@/lib/services/gitlab/repository";
...
export default function RepositoryMembersDisplayer({
  ...
  const [info, setInfo] = useState<TMember[]>([]);
          ...
          <TableRow key={item.id}>
            ...
            <TableCell>
              <div className="flex text-center justify-center items-center gap-2 font-thin">
                {user.role == 'admin' ? (item.username == user.username ? <></> : <LogOut onClick={() => { deleteUserFromRepository(item.id, repositoryId).then(_=> {refreshMemberList()}) }} size={18} className="text-red-500" />) : <EyeIcon size={18} className="text-blue-400" />}
              </div>
            </TableCell>
          </TableRow>
        )}
